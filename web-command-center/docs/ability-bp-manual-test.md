...
- 独立审查修复：已通过可执行渲染决策测试，警告成立时不会再同时显示绿色普通比赛或 `.start` 正式开赛指引；本次修复后未重新进行浏览器实测。
...
脚本 cachebuster 已由浏览器确认过的 `ability-bp-task9-final-20260720` 更新为 `ability-bp-task9-start-guard-20260720`。下文最后一次浏览器确认仍是前一个 `final` 脚本版本的真实证据；不将其写成本次审查修复后的浏览器验证。
...
| 插件同步提示 | `final` 脚本浏览器通过：管理员端固定警告 count=1、visible=true；新加入的匿名临时参赛者 Charlie 端 count=1、visible=true。独立审查后的 `start-guard` 脚本由可执行测试确认警告成立时隐藏普通比赛和 `.start` 指引；异能关闭、duel 与不完整分配保留旧路径 |
| assignment 完整性 | 自动化通过：playerId、team 与非空 abilityId 必须同时有效；错队或空 abilityId 不显示警告，也不屏蔽旧路径 |
| 控制台 | `final` 脚本浏览器通过：过滤后的控制台 error 为空；本次 `start-guard` 修复后未重新进行浏览器实测 |
