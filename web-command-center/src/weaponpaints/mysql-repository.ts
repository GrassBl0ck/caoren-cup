...
export class MySqlLoadoutRepository implements LoadoutRepository {
    ...
    async copyTeam(steamId: string, fromTeam: 2 | 3, toTeam: 2 | 3, audit: SkinAuditEntry): Promise<void> {
        await this.transaction(async (connection) => {
            ...
            await connection.execute(
                ...
                [toTeam, steamId, fromTeam],
            );
            await connection.execute(
                `INSERT INTO \`weapon_stickers\` (\`steamid\`,\`team\`,\`weapon_defindex\`,\`slot\`,\`sticker_id\`,\`sticker_schema\`,\`offset_x\`,\`offset_y\`,\`wear\`,\`scale\`,\`rotation\`)
            ...
            await connection.execute(
                `INSERT INTO \`player_cosmetics\` (\`steamid\`,\`team\`,\`kind\`,\`item_key\`)
                 SELECT \`steamid\`,?,\`kind\`,\`item_key\` FROM \`player_cosmetics\` WHERE \`steamid\`=? AND \`team\`=?
