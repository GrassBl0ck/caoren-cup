...
test('schema v2 保存完整赛前流程，且不保存撤销历史、登录秘密和旧大厅字段', () => {
    clearFlowUndoHistory();
    const session = createInitialSession();
    session.phase = GamePhase.MapBan;
    session.players.admin = { ...snapshotAdmin, bindCode: 'BIND-SECRET', sessionCode: 'SESSION-SECRET' };
    ...
    const payload = persistence.buildSessionSnapshotPayload(session);
    ...
    assert.equal(persistence.restoreSessionSnapshotData(legacyPayload), true);
...
test('schema v1 恢复当前赛前流程时清空进程内撤销历史', () => {
    clearFlowUndoHistory();
    const current = createInitialSession();
    current.players.admin = { ...snapshotAdmin };
    ...
    assert.equal(persistence.restoreSessionSnapshotData({
        version: 1,
        savedAt: Date.now(),
        session: { ...current, phase: GamePhase.CaptainSelection, rollTimeout: undefined },
    }), true);
...
test('未知快照版本不会替换当前状态', () => {
    ...
    assert.equal(persistence.restoreSessionSnapshotData({ version: 99, session: {} }), false);
