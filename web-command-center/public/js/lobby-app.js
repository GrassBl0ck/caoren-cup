...
const ws = io();
        ...
        function applyTheme(theme) {
            ...
            if (btn) btn.textContent = value === 'dark' ? '????' : '????';
        }
        ...
        function getTimerSecondsText() {
            if (!currentTimerEndAt) return '-- ?';
            return Math.max(0, Math.ceil((currentTimerEndAt - syncedNow()) / 1000)) + ' ?';
        }
        ...
        function updateTimerDisplay() {
            ...
            if (!currentTimerEndAt) {
                display.textContent = '';
                document.querySelectorAll('.inline-timer-text').forEach(el => el.textContent = '-- ?');
                if (countdownInterval) { clearInterval(countdownInterval); countdownInterval = null; }
                return;
            }
            const remaining = Math.max(0, Math.ceil((currentTimerEndAt - syncedNow()) / 1000));
            display.textContent = '? ?????' + remaining + ' ?';
            document.querySelectorAll('.inline-timer-text').forEach(el => el.textContent = remaining + ' ?');
            if (remaining <= 0) {
        ...
        function getExpectedSide(player, state) {
            if (!player || (player.rosterTeam !== 'A' && player.rosterTeam !== 'B')) {
                return { side: null, label: '???', reason: 'unassigned' };
            }

            const teamASide = getTeamASideForState(state);
            if (teamASide !== 'CT' && teamASide !== 'T') {
                return { side: null, label: '???', reason: 'pending-side-pick' };
            }
        ...
        function renderSideTag(side, emptyLabel = '???') {
        ...
        function getAdminPasswordForRequest() {
            return document.getElementById('extra-input')?.value || prompt('?????????') || '';
        }
        ...
        function syncMatchOptionsPanel(state, isAdmin) {
            ...
            if (status) {
                status.innerHTML = `?????<b>${phaseDisplayName(phase)}</b>?${editable ? '?????????' : '????????'}`;
            }
            if (undercoverInput) {
        ...
        function syncCaorenModPanel(state, isAdmin, caorenEnabled) {
            ...
            if (status) {
                if (!caorenEnabled) {
                    ...
                    status.innerHTML = `?????<b>${phaseDisplayName(phase)}</b>?CaorenCup ????????????????????????????? CS2 ???????`;
                } else {
                    status.innerHTML = `?????<b>${phaseDisplayName(phase)}</b>?CaorenCup ???????????????????????????`;
                }
            }
...
if (window._caorenModifiersEnabled !== true) {
            ...
            if (module === 'reset_all' && !confirm('?????? CaorenCup ?????????? reset_plu?')) {
                return;
            }
            ...
            try {
                ...
                const data = await res.json();
                if (!res.ok || !data.success) throw new Error(data.error || 'CaorenCup ??????');

                const status = document.getElementById('caoren-mod-status');
                if (status) {
                    status.innerHTML = `????????<b>${data.label}</b><br>???????<code>${data.command}</code><br>?????????????????`;
                }
                alert('????????' + data.label);
            } catch (err) {
                alert(err.message || 'CaorenCup ??????');
            }
        }
        ...
        async function syncTeamLock() {
            ...
            try {
                ...
                const missing = Array.isArray(data.missingSteamIds) && data.missingSteamIds.length
                    ? ` ????${data.missingSteamIds.map(p => p.name).join('?')}?`
                    : '';
                const unassigned = Array.isArray(data.unassignedPlayers) && data.unassignedPlayers.length
                    ? ` ????${data.unassignedPlayers.map(p => p.name).join('?')}?`
                    : '';
                if (status) status.textContent = `????????${data.assignments.length} ?????? ${data.round || 0}${data.halftimeSwapped ? '????????' : ''}?${missing}${unassigned}`;
            } catch (err) {
        ...
        ws.on('LOGIN_RESPONSE', (data) => {
            if (data.success) {
                ...
                const displayName = data.name || data.player?.name || (data.message || '').split('?')[0].replace('???', '').replace('???????????', '').replace('???????:', '').trim();
                document.getElementById('my-name').textContent = displayName || document.getElementById('name-input').value.trim() || '?????';
                document.getElementById('my-bindcode').textContent = data.loginCode || data.bindCode || '???';
                const codeLine = document.getElementById('my-bindcode')?.parentElement;
                if (codeLine && !data.loginCode && data.bindCode) {
                    codeLine.childNodes[2].textContent = '????????';
                }
            } else {
                if (data.resetClient || data.message === '??????' || String(data.message || '').includes('??')) {
                    resetToLogin(data.message || '????????????');
                } else {
                    alert('?????' + data.message);
                }
            }
        ...
        ws.on('GAME_STATE', (state) => {
            ...
            if (currentPlayer) {
                let identityHtml = '???<b>' + currentPlayer.name + '</b>';
                if (isAdmin) identityHtml += ' <span style="color:#d32f2f;">(???)</span>';
                else {
                    if (currentPlayer.rosterTeam) identityHtml += ' | <span style="font-weight:bold;">?????' + currentPlayer.rosterTeam + '?</span>';
                    const currentSide = getLiveSide(currentPlayer) || '???';
                    const expectedSide = getExpectedSide(currentPlayer, state).label;
                    const mismatchText = isSideMismatch(currentPlayer, state) ? ' <span style="color:#d32f2f; font-weight:bold;">???</span>' : '';
                    identityHtml += ' | <span style="font-weight:bold;">????' + currentSide + '</span>';
                    identityHtml += ' | <span style="font-weight:bold;">????' + expectedSide + '</span>' + mismatchText;

                    if (currentPlayer.steamIdBound) identityHtml += ' | <span style="color:#2e7d32; font-weight:bold;">???</span>';
                    else identityHtml += ' | <span style="color:#9ca3af; font-weight:bold;">???</span>';
                    if (currentPlayer.gameRole) identityHtml += ' | ??: ' + currentPlayer.gameRole;
                    else if (state.phase === 'PreGameSetup' && !state.rolesReleased) identityHtml += ' | ?????';
                }
            ...
            let playerTable = '<h3 style="margin-top:0;">??????</h3>';
            playerTable += '<div class="cc-table-wrap"><table class="cc-table"><thead><tr>' +
                '<th>#</th><th>???</th><th>????</th><th>???</th><th>???</th><th>????</th><th>??</th><th>??</th>' + (isAdmin ? '<th>??</th>' : '') +
                '</tr></thead><tbody>';
            visiblePlayers.forEach((p, idx) => {
                ...
                const roster = p.rosterTeam ? `<span class="tag tag-gray">${p.rosterTeam}?</span>` : '<span class="tag tag-gray">???</span>';
                const side = renderSideTag(getLiveSide(p));
                const expectedSide = renderExpectedSideTag(p, state);
                const bind = p.steamIdBound ? '<span class="tag tag-green">???</span>' : '<span class="tag tag-red">???</span>';
                const roleText = p.role === 'Admin' ? '<span class="tag tag-purple">???</span>' : (p.gameRole ? `<span class="tag tag-gray">${p.gameRole}</span>` : '<span class="tag tag-gray">???</span>');
                const ready = renderReadyTag(p);
                const adminOps = isAdmin && p.role !== 'Admin' ? `<td><button onclick="kickPlayer('${p.playerId}', '${htmlEscape(p.name)}')" style="background:#b91c1c;color:#fff;padding:4px 9px;">??</button></td>` : (isAdmin ? '<td>-</td>' : '');
            ...
            if (isAdmin) {
                ...
                if (btn) btn.textContent = '???? (??: ' + state.phase + ')';
            ...
            phaseDiv.innerHTML = '<div class="section-title"><div><div class="status-chip">????</div><div style="margin-top:10px;font-size:20px;font-weight:800;">?????' + state.phase + '</div></div></div>';
            ...
            if (state.phase === 'Lobby') {
                ...
                if (isAdmin) {
                    if (undercoverEnabled) {
                        html += '<div style="background:#e3f2fd; padding:15px; border-radius:5px; border:1px solid #90caf9;"><b>???????</b><br><br>' +
                            '????(??)?<input id="undercover-count" type="number" value="' + state.undercoverCount + '" min="0" style="width:80px;"> &nbsp;&nbsp;' +
                            '????(??)?<input id="detective-count" type="number" value="' + state.detectiveCount + '" min="0" style="width:80px;"> &nbsp;' +
                            '<button onclick="updateRoleCounts()" style="background:#2196f3; color:#fff;">????</button></div>';
                    } else {
                        html += '<div style="background:#e8f5e9; padding:15px; border-radius:5px; border:1px solid #a5d6a7;"><b>???????</b>????????????????/???????????????????????</div>';
                    }
                }
                html += '<hr><button onclick="confirmQuit()" style="color:#d32f2f; border-color:#d32f2f;">????</button>';
            ...
            if (state.phase === 'CaptainSelection') {
                ...
                extraDiv.innerHTML = '<h3>?? ????</h3><p>A??????<b>' + (capA ? capA.name : '?') + '</b></p><p>B??????<b>' + (capB ? capB.name : '?') + '</b></p>';
                if (isAdmin) {
                    extraDiv.innerHTML += '<hr><div style="background:#fff3e0; padding:10px; border:1px solid #ffcc80;"><b>????????</b><br>' +
                        '<button onclick="rerandomCaptain(\'A\')">?? A?</button> <button onclick="rerandomCaptain(\'B\')">?? B?</button><br><br>' +
                        '???? A?: <select id="capA-select">' + Object.values(state.players).filter(p => p.playerId !== state.captains.B && p.role !== 'Admin').map(p => '<option value="' + p.playerId + '">' + p.name + '</option>').join('') + '</select> ' +
                        '<button onclick="setCaptain(\'A\')">??</button><br>' +
                        '???? B?: <select id="capB-select">' + Object.values(state.players).filter(p => p.playerId !== state.captains.A && p.role !== 'Admin').map(p => '<option value="' + p.playerId + '">' + p.name + '</option>').join('') + '</select> ' +
                        '<button onclick="setCaptain(\'B\')">??</button></div>';
                }
            }
            ...
            if (state.phase === 'Roll') {
                ...
                if (isCaptain && !alreadyRolled) {
                    extraDiv.innerHTML = '<button onclick="doRoll()" style="font-size:24px; padding:15px 30px; background:#4caf50; color:#fff; border-radius:8px;">?? ?????</button>';
                }
                const rollA = state.rollValues.A !== null ? `<span style="color:red;font-size:20px">${state.rollValues.A}</span>` : '??';
                const rollB = state.rollValues.B !== null ? `<span style="color:blue;font-size:20px">${state.rollValues.B}</span>` : '??';
                extraDiv.innerHTML += '<p style="font-size:18px;">A?? (' + (state.players[state.captains.A]?.name || '') + ') ? ' + rollA + '</p>';
                extraDiv.innerHTML += '<p style="font-size:18px;">B?? (' + (state.players[state.captains.B]?.name || '') + ') ? ' + rollB + '</p>';
                extraDiv.innerHTML += '<p style="color:#666;">???????????????BP??????????</p>';
            }
            ...
            if (state.phase === 'PlayerDraft') {
                ...
                html += '<div><h2 class="draft-title">????</h2>';
                html += `<p class="draft-subtitle">??????? ${batchTotal || 0} ???? ${batchDone || 0} ???? ${batchLeft || 0} ????????????? 1 ?????????</p></div>`;
                html += `<div class="map-bp-turn-wrap">
                            <div class="draft-turn-card"><span style="display:block;color:#607086;font-size:13px;margin-bottom:6px;">????</span><b>${currentTeam || '-'}?</b><div style="font-size:13px;color:#607086;margin-top:4px;">???${currentCaptain?.name || '-'}</div></div>
                            <div class="bp-inline-timer"><span>?????</span><strong class="inline-timer-text">${getTimerSecondsText()}</strong></div>
                         </div>`;
                html += '</div>';

                html += '<div class="map-bp-summary">';
                html += `<div class="map-bp-summary-card"><span>????</span><strong>${donePicks} / ${totalPicks}</strong></div>`;
                html += `<div class="map-bp-summary-card"><span>???</span><strong>${currentTeam || '-'}? ${batchDone}/${batchTotal}</strong></div>`;
                html += `<div class="map-bp-summary-card"><span>A ???</span><strong>${capA?.name || '-'}</strong></div>`;
                html += `<div class="map-bp-summary-card"><span>B ???</span><strong>${capB?.name || '-'}</strong></div>`;
                html += `<div class="map-bp-summary-card"><span>????</span><strong>${isMyTurn ? '?????' : (currentTeam ? '??????' : '????')}</strong></div>`;
                html += '</div>';

                if (available.length > 0) {
                    html += '<h3 style="margin-top:18px;">????</h3>';
                    html += '<div class="draft-pick-grid">';
                    available.forEach(p => {
                        html += `<div class="draft-player-card ${isMyTurn ? 'pickable' : ''}" ${isMyTurn ? `onclick="pick('${p.playerId}')"` : ''}>
                                    <strong>${p.name}</strong>
                                    <span class="map-card-status ${isMyTurn ? 'available' : 'waiting'}">${isMyTurn ? '????' : '????'}</span>
                                    ${isAdmin ? `<div style="margin-top:10px; display:flex; gap:6px; flex-wrap:wrap;"><button onclick="event.stopPropagation(); adminAssignTeam('${p.playerId}', 'A')" style="background:#f97316;color:#fff;">??A</button><button onclick="event.stopPropagation(); adminAssignTeam('${p.playerId}', 'B')" style="background:#2563eb;color:#fff;">??B</button></div>` : ''}
                                 </div>`;
                    });
                    html += '</div>';
                } else {
                    html += '<div class="soft-block" style="margin-top:14px;">????????????? BP?</div>';
                }

                html += '<div class="draft-team-panels">';
                html += '<div class="draft-team-panel a"><b>A???</b><br><br>' + state.teams.A.players.map(id => state.players[id]?.name || '-').join('<br>') + '</div>';
                html += '<div class="draft-team-panel b"><b>B???</b><br><br>' + state.teams.B.players.map(id => state.players[id]?.name || '-').join('<br>') + '</div>';
                html += '</div>';
                html += '</div>';
                extraDiv.innerHTML = html;
            }
            ...
            if (state.phase === 'MapBan') {
                ...
                html += '<div><h2 class="map-bp-title">?? BP</h2>';
                html += `<p class="map-bp-subtitle">???????????????????????????????? Ban ????????</p></div>`;
                html += `<div class="map-bp-turn-wrap">
                            <div class="map-bp-turn"><span style="display:block;color:#607086;font-size:13px;margin-bottom:6px;">????</span><b>${currTeam || '-'}? Ban ?</b></div>
                            <div class="bp-inline-timer"><span>???</span><strong class="inline-timer-text">${getTimerSecondsText()}</strong></div>
                         </div>`;
                html += '</div>';

                html += '<div class="map-bp-summary">';
                html += `<div class="map-bp-summary-card"><span>????</span><strong>${available.length > 0 ? available.join(' / ') : '?'}</strong></div>`;
                html += `<div class="map-bp-summary-card"><span>? Ban</span><strong>${state.bannedMaps.length > 0 ? state.bannedMaps.join(' / ') : '??'}</strong></div>`;
                html += `<div class="map-bp-summary-card"><span>????</span><strong>${canVote ? '???????????/????' : (myVotedMap ? '????' + myVotedMap + '?????' : '????????')}</strong></div>`;
                html += '</div>';

                if (state.mapVote) {
                    html += '<div class="soft-block" style="margin-bottom:14px;">';
                    html += `<b>${currTeam}??????</b>`;
                    html += voteItems ? `<ul class="map-vote-list">${voteItems}</ul>` : '<span style="color:#64748b;margin-left:8px;">????</span>';
                    html += '</div>';
                }
                ...
                if (isAdmin) {
                    html += '<div class="map-admin-ban-row"><b>????????</b><span style="color:#64748b;font-size:13px;"> ??? Ban ????????</span><br>';
                    available.forEach(m => {
                        html += `<button onclick="adminBanMap('${m}')" style="background:#1f2937; color:#fff;">? Ban ${m}</button> `;
                    });
                    html += '</div>';
                }

                html += '<div class="map-thumb-spec"><b>??????</b>??????? <code>public/assets/maps/</code>??? 640?360px?16:9?JPG/WebP/PNG ?????????? <code>de_mirage.jpg</code>?<code>de_inferno.jpg</code>?<code>de_dust2.jpg</code> ?????</div>';
                html += '</div>';
                extraDiv.innerHTML = html;
            }
            ...
            if (state.phase === 'SidePick') {
                ...
                let html = '<div class="map-bp-board">';
                html += '<div class="map-bp-header">';
                html += '<div><h2 class="map-bp-title">????</h2>';
                html += `<p class="map-bp-subtitle">?????<b>${state.selectedMap || '?????'}</b>?${voteTeam}??????????????????????????????????</p></div>`;
                html += `<div class="map-bp-turn-wrap">
                            <div class="map-bp-turn"><span style="display:block;color:#607086;font-size:13px;margin-bottom:6px;">??????</span><b>${voteTeam}?</b></div>
                            <div class="bp-inline-timer"><span>???</span><strong class="inline-timer-text">${getTimerSecondsText()}</strong></div>
                         </div>`;
                html += '</div>';

                html += '<div class="map-bp-summary">';
                html += `<div class="map-bp-summary-card"><span>CT ??</span><strong>${ctVotes}</strong></div>`;
                html += `<div class="map-bp-summary-card"><span>T ??</span><strong>${tVotes}</strong></div>`;
                html += `<div class="map-bp-summary-card"><span>????</span><strong>${canSideVote ? (mySideVote ? '????' + mySideVote + '?????' : '?????????') : '????????'}</strong></div>`;
                html += '</div>';
                ...
                html += `<div class="map-card ${canSideVote ? 'clickable' : ''} ${mySideVote === 'CT' ? 'selected-map' : ''}" ${canSideVote ? "onclick=\"selectSide('CT')\"" : ''}>
                            ...
                            <div class="map-card-body">
                                <div class="map-card-name"><strong>?? CT</strong><span class="map-card-slug">Counter-Terrorist</span></div>
                                <span class="map-card-status ${mySideVote === 'CT' ? 'selected' : (canSideVote ? 'available' : 'waiting')}">${mySideVote === 'CT' ? '????' : (canSideVote ? '????' : '??')}</span>
                            </div>
                         </div>`;
                html += `<div class="map-card ${canSideVote ? 'clickable' : ''} ${mySideVote === 'T' ? 'selected-map' : ''}" ${canSideVote ? "onclick=\"selectSide('T')\"" : ''}>
                            ...
                            <div class="map-card-body">
                                <div class="map-card-name"><strong>?? T</strong><span class="map-card-slug">Terrorist</span></div>
                                <span class="map-card-status ${mySideVote === 'T' ? 'selected' : (canSideVote ? 'available' : 'waiting')}">${mySideVote === 'T' ? '????' : (canSideVote ? '????' : '??')}</span>
                            </div>
                         </div>`;
                html += '</div>';
                html += '<div class="side-thumb-spec"><b>??????</b>??? <code>public/assets/sides/</code>??? <code>ct.jpg</code> ? <code>t.jpg</code>????????640?360px?16:9????????????????</div>';
            ...
            if (state.phase === 'PreGameSetup') {
                ...
                if (!undercoverEnabled) {
                    if (isAdmin) {
                        html += '<div style="background:#e8f5e9; padding:15px; border:1px solid #a5d6a7; border-radius:8px;"><h4 style="margin-top:0;color:#2e7d32;">??????</h4><p>????????????????/???????????????????????</p><p style="margin-bottom:0;">??????????????????????????</p></div><hr>';
                    }

                    html += '<h3 style="text-align:center; color:#2e7d32;">?????????</h3>';
                    html += '<p style="text-align:center; color:#607086;">?????????????????????????????????</p>';
                    ...
                    if (currentPlayer?.role !== 'Admin' && !currentPlayer?.isReady) {
                        html += '<button onclick="readyPlayer()" style="font-size:24px; padding:15px 40px; background:#4caf50; color:#fff; border-radius:8px; box-shadow:0 4px 6px rgba(0,0,0,0.2);">?? ???????????</button>';
                    } else {
                        html += '<h3 style="color:#4caf50;">? ?????????????</h3>';
                    }
                    html += '<p style="color:#888; font-size:14px; margin-top:15px;">??????' + Object.values(state.players).filter(p => p.isReady).map(p => p.name).join(', ') + '</p></div>';
                    extraDiv.innerHTML = html;
                } else {
                    if (isAdmin) {
                        html += '<div style="background:#f3e5f5; padding:15px; border:1px solid #ce93d8;"><h4>??? ?????????</h4>';
                        html += '<table border=1 cellpadding=6 style="width:100%; border-collapse:collapse; background:#fff; text-align:center;"><tr><th>??</th><th>????</th><th>???</th><th>???</th><th>??</th><th>????</th><th>??</th><th>\u4efb\u52a1\u786e\u8ba4</th></tr>';
                        Object.values(state.players).forEach(p => {
                            if (p.role === 'Spectator' || p.role === 'Admin') return;
                            const curRole = p.gameRole || '<span style="color:red">???</span>';
                            const liveSide = getLiveSide(p) || '???';
                            const expected = getExpectedSide(p, state).label;
                            const mismatchStyle = isSideMismatch(p, state) ? ' style="background:#fff7ed;"' : '';
                            html += `<tr${mismatchStyle}><td><b>${p.name}</b></td><td>${p.rosterTeam ? p.rosterTeam + '?' : '???'}</td><td>${liveSide}</td><td>${expected}</td><td>${p.steamIdBound ? '???' : '???'}</td><td>${curRole}</td>`;
                            html += `<td><select id="role-select-${p.playerId}">
                                    <option value="Undercover" ${p.gameRole === 'Undercover' ? 'selected' : ''}>??</option>
                                    <option value="Detective" ${p.gameRole === 'Detective' ? 'selected' : ''}>??</option>
                                    <option value="Soldier" ${p.gameRole === 'Soldier' ? 'selected' : ''}>??</option>
                                    </select> <button onclick="setPlayerRole('${p.playerId}')">??</button></td><td>${p.gameRole === 'Undercover' ? renderUndercoverAckTag(p) : '-'}</td></tr>`;
                        });
                        html += '</table>';
                        html += `<p style="margin-top:10px;">????(??)?${state.undercoverCount} ??, ${state.detectiveCount} ??</p>`;
                        html += '<button onclick="randomRemainingRoles()" style="background:#7c3aed; color:#fff; font-weight:bold;">????????????????</button> ';
                        html += '<button onclick="releaseRoles()" style="background:#d32f2f; color:#fff; font-weight:bold;">???????</button>';
                        html += `<p style="font-size:13px;color:#6b7280;margin-bottom:0;">?????${state.rolesReleased ? '<b style="color:#2e7d32;">???</b>' : '<b style="color:#d32f2f;">???</b>'}????????????????????</p></div><hr>`;
                    }
                    if (role) {
                        let roleStyle = role === 'Undercover' ? 'color:#d32f2f' : (role === 'Detective' ? 'color:#1976d2' : 'color:#388e3c');
                        html += `<h2 style="text-align:center;">????????<span style="${roleStyle}; font-size:32px;">${role}</span></h2>`;
                        html += '<div style="text-align:center;"><button onclick="showRules()" style="font-size:18px; padding:10px 20px;">?? ???????????</button></div>';
                        ...
                        html += '<h3 style="text-align:center; color:#666;">?????????????????????????</h3>';
                    }
            ...
            if (state.phase === 'MidGameQA') {
                if (!undercoverEnabled) {
                    extraDiv.innerHTML = '<div style="background:#e8f5e9; padding:20px; border:1px solid #a5d6a7; border-radius:8px;"><h2 style="margin-top:0;color:#2e7d32;">???????</h2><p>???????????????????????????????????????</p></div>';
                } else {
                    extraDiv.innerHTML = '<div style="background:#e3f2fd; padding:20px; border:1px solid #90caf9; border-radius:8px;"><h2 style="margin-top:0;color:#1565c0;">????????????</h2><p>??????????????????? LiveGame ????????????????????????????? LiveGame ????????????????????? -12 ???</p></div>';
                }
            }
            ...
            if (state.phase === 'PostGameAccusation') {
                ...
                html += '<h2 style="color:#e65100; margin-top:0;">?? ??????</h2>';
                html += '<p style="font-size:16px;">????????????????????????</p>';

                const myAcc = state.accusations[myPlayerId] || { own: null, enemy: null };
                if (!isAdmin && currentPlayer) {
                    if (!myAcc.own) {
                        html += '<div class="accuse-area" style="background:#e8f5e9; padding:15px; border:1px solid #c8e6c9;"><b>?? ???????</b><br><br>';
                        Object.values(state.players).forEach(p => {
                            if (p.playerId === myPlayerId || p.role === 'Spectator' || p.role === 'Admin') return;
                            if (p.rosterTeam === currentPlayer.rosterTeam) {
                                html += `<button onclick="accuse('${p.playerId}', 'own')" style="font-size:16px; padding:8px 15px;">?? ${p.name}</button> `;
                            }
                        });
                        html += '</div>';
                    } else {
                        html += `<p style="font-size:16px; color:#2e7d32;">? ?????????? <b>${state.players[myAcc.own]?.name || ''}</b></p>`;
                    }

                    if (!myAcc.enemy) {
                        html += '<div class="accuse-area" style="background:#ffebee; padding:15px; border:1px solid #ffcdd2; margin-top:15px;"><b>?? ???????</b><br><br>';
                        Object.values(state.players).forEach(p => {
                            if (p.playerId === myPlayerId || p.role === 'Spectator' || p.role === 'Admin') return;
                            if (p.rosterTeam !== currentPlayer.rosterTeam) {
                                html += `<button onclick="accuse('${p.playerId}', 'enemy')" style="font-size:16px; padding:8px 15px;">?? ${p.name}</button> `;
                            }
                        });
                        html += '</div>';
                    } else {
                        html += `<p style="font-size:16px; color:#c62828;">? ?????????? <b>${state.players[myAcc.enemy]?.name || ''}</b></p>`;
                    }
                } else {
                    html += '<p style="color:#666;">????????????</p>';
                }
                ...
                for (const [pId, acc] of Object.entries(state.accusations)) {
                    ...
                    const ownText = acc.own ? '????' : '???';
                    const enemyText = acc.enemy ? '????' : '???';
                    html += `<p style="margin:5px 0; color:#555;">?? ${p.name} - ???: ${ownText} | ???: ${enemyText}</p>`;
                }
                html += '</div>';
                extraDiv.innerHTML = html;
            ...
            if (state.phase === 'Scoreboard') {
                ...
                const winnerText = live.winnerTeam ? `????${live.winnerTeam}?` : '';
                let html = `<h2 style="color:#fbc02d; text-align:center; font-size:32px;">?? ????</h2><p style="text-align:center;font-size:18px;">?????A? ${live.scoreA || 0} : ${live.scoreB || 0} B? ${winnerText}</p>`;
                ...
                if (Object.values(state.players).some(p => p.finalScore !== undefined)) {
                    ...
                    if (undercoverEnabled) {
                        html += '<table class="score-detail-table"><tr><th>??</th><th>??</th><th>??</th><th>??</th><th>??</th><th>??</th><th>????</th><th>????</th><th>??</th><th>?? / ??</th><th>?? / ??</th><th>????</th></tr>';
                    } else {
                        html += '<table class="score-detail-table"><tr><th>??</th><th>??</th><th>??</th><th>??</th><th>??</th><th>??</th><th>????</th><th>????</th><th>??</th><th>????</th></tr>';
                    }
                    ...
                    html += '</table></div>';
                    html += '<div class="score-rule-note">????????????????????????????????????????????????? 1 ???????????? 2 ??????? 0 ??????</div>';                    if (isAdmin) {
                        html += '<div class="score-rule-note">MatchZy CSV ?????????????????????????</div>';
                    }
                } else {
                    html += '<div style="text-align:center; padding:30px; background:#f5f5f5; border-radius:8px;"><p style="font-size:18px; color:#666;"><i>??????????????...</i></p>';                    if (isAdmin) {
                        html += '<div class="score-rule-note">???????????MatchZy CSV ??????????????</div>';
                    }
                    html += '</div>';
                }
                ...
                if (undercoverEnabled && state.accusations && Object.keys(state.accusations).length > 0) {
                    html += '<h4 style="margin-top:20px;">??? ???????????</h4><div style="background:#fafafa; padding:15px; border:1px solid #ddd; border-radius:5px; column-count:2; font-size:13px;">';
                    for (const [pId, acc] of Object.entries(state.accusations)) {
                        const p = state.players[pId];
                        if (!p || p.role === 'Admin') continue;
                        const ownName = acc.own ? state.players[acc.own]?.name : '-';
                        const enemyName = acc.enemy ? state.players[acc.enemy]?.name : '-';
                        html += `<p style="margin:5px 0;"><b>${p.name}</b> ??? <span style="color:#2e7d32;">${ownName}(?)</span> ? <span style="color:#c62828;">${enemyName}(?)</span></p>`;
                    }
                    html += '</div>';
                }
                extraDiv.innerHTML = html;
        ...
        function renderRoundScoreTerms(role, p, state) {
            ...
            if (role === 'Undercover') {
                ...
                return formulaTerm('???', -4, rr.won, winScore) +
                    formulaTerm('???', 10, rr.lost, loseScore) +
                    singleScoreTerm('??', getScoreBreakdownValue(p, '????'));
            }
            if (role === 'Detective') {
                const winScore = rr.won * 8;
                const loseScore = rr.lost * -4;
                return formulaTerm('???', 8, rr.won, winScore) +
                    formulaTerm('???', -4, rr.lost, loseScore) +
                    singleScoreTerm('??', getScoreBreakdownValue(p, '????'));
            }
            const winScore = rr.won * 10;
            const loseScore = rr.lost * -4;
            return formulaTerm('???', 10, rr.won, winScore) +
                formulaTerm('???', -4, rr.lost, loseScore) +
                singleScoreTerm('??', getScoreBreakdownValue(p, '????'));
        }
        ...
        function renderScoreboardRow(p, state) {
            ...
            if (!undercoverEnabled) {
                const killCell = formulaTerm('', 5, kills, getScoreBreakdownValue(p, '??'));
                const deathCell = formulaTerm('', -2, deaths, getScoreBreakdownValue(p, '??'));
                const assistCell = formulaTerm('', 2, assists, getScoreBreakdownValue(p, '??'));
                const gameCell = singleScoreTerm('', getScoreBreakdownValue(p, '????'));
                const roundCell = renderRoundScoreTerms('Soldier', p, state);
                const damageCell = formulaTerm('', 1, damageUnits, getScoreBreakdownValue(p, '??')) + scoreTerm('????', countSpan(damage));
            ...
            if (role === 'Soldier') {
                killCell = formulaTerm('', 5, kills, getScoreBreakdownValue(p, '??'));
                deathCell = formulaTerm('', -2, deaths, getScoreBreakdownValue(p, '??'));
                assistCell = formulaTerm('', 2, assists, getScoreBreakdownValue(p, '??'));
                gameCell = singleScoreTerm('', getScoreBreakdownValue(p, '????'));
                roundCell = renderRoundScoreTerms(role, p, state);
                damageCell = formulaTerm('', 1, damageUnits, getScoreBreakdownValue(p, '??')) + scoreTerm('????', countSpan(damage));
                accuseCell = formulaTerm('????', 15, getScoreBreakdownValue(p, '??????'), getScoreBreakdownValue(p, '????'));
                taskCell = emptyScoreTerm('???');
            } else if (role === 'Undercover') {
                killCell = formulaTerm('', -2, kills, getScoreBreakdownValue(p, '??'));
                deathCell = formulaTerm('', 5, deaths, getScoreBreakdownValue(p, '??'));
                assistCell = formulaTerm('', -1, assists, getScoreBreakdownValue(p, '??'));
                gameCell = singleScoreTerm('', getScoreBreakdownValue(p, '????'));
                roundCell = renderRoundScoreTerms(role, p, state);
                damageCell = formulaTerm('', -0.75, damageUnits, getScoreBreakdownValue(p, '??')) + scoreTerm('????', countSpan(damage));
                accuseCell = formulaTerm('???', -5, getScoreBreakdownValue(p, '?????'), getScoreBreakdownValue(p, '???')) +
                    singleScoreTerm('????', getScoreBreakdownValue(p, '????'));
                taskCell = singleScoreTerm('????', getScoreBreakdownValue(p, '????')) +
                    formulaTerm('??', 14, getScoreBreakdownValue(p, '???'), getScoreBreakdownValue(p, '??')) +
                    singleScoreTerm('????', getScoreBreakdownValue(p, '??'));
            } else if (role === 'Detective') {
                killCell = formulaTerm('', 4, kills, getScoreBreakdownValue(p, '??'));
                deathCell = formulaTerm('', -2, deaths, getScoreBreakdownValue(p, '??'));
                assistCell = formulaTerm('', 2, assists, getScoreBreakdownValue(p, '??'));
                gameCell = singleScoreTerm('', getScoreBreakdownValue(p, '????'));
                roundCell = renderRoundScoreTerms(role, p, state);
                damageCell = formulaTerm('', 0.9, damageUnits, getScoreBreakdownValue(p, '??')) + scoreTerm('????', countSpan(damage));
                accuseCell = formulaTerm('????', 20, getScoreBreakdownValue(p, '??????'), getScoreBreakdownValue(p, '????')) +
                    scoreTerm('???', countSpan(getScoreBreakdownValue(p, '?????'))) +
                    singleScoreTerm('????', getScoreBreakdownValue(p, '????'));
                taskCell = emptyScoreTerm('???');
            }
        ...
        function renderMapCard(map, state, options) {
            ...
            const statusText = selected ? '????' : (banned ? '? Ban' : (myVote ? '????' : (canVote ? '????' : '??')));
            ...
            if (!banned && !selected && state.mapVote) {
                if (voters.length > 0) {
                    votersHtml = voters.map(v => {
                        ...
                        return `<span class="map-voter-pill ${isMe ? 'me' : ''}"><span class="dot"></span>${isMe ? '?' : v.name}</span>`;
                    }).join('');
                    votersHtml += `<span class="map-card-vote-count">${voters.length}?</span>`;
                } else {
                    votersHtml = '<span class="map-card-voters-empty">?????????</span>';
                }
            }

            return `
                <div class="${cardClass}" ${click} title="${canVote ? '?? Ban ' + escapeAttr(map) : escapeAttr(map)}">
        ...
        function renderTaskCell(cellId, cell, options = {}) {
            ...
            const desc = cell ? htmlEscape(cell.description) : '?';
            const hasN = cell && cell.nType !== 'none' && Number(cell.nValue || 0) > 0;
            const nInfo = hasN ? `<div style="color:#d32f2f; font-size:${compact ? '12px' : '15px'}; margin-top:8px; font-weight:bold; background:#ffebee; padding:4px; border-radius:4px; width:100%;">?? N = ${cell.nValue}</div>` : '';
            const roundInfo = cell?.completedRound ? `<div class="task-round-note">?????? ${cell.completedRound} ??</div>` : '';
        ...
        function renderHltvTeamTable(teamName, rows) {
            ...
            html += '<tr>';
            html += thTip('Players', '??');
            html += thTip('<span>Op.</span>K-D', '????? : ???');
            html += thTip('MKs', '??????2K/3K/4K/5K?');
            html += thTip('KAST', '?? KAST???????????? HLTV ????');
            html += thTip('1vsX', '???????1v1 ? 1v6');
            html += thTip('K (hs)', '??????????');
            html += thTip('A (f)', '???????????????');
            html += thTip('D (t)', '???????????????');
            html += thTip('ADR', '???????');
            html += thTip('Swing', '?????????? XvX?????????');
            html += thTip('Rating', '???? Rating????? HLTV Rating', '<span>3.0*</span>');
            html += '</tr></thead><tbody>';
            rows.forEach(row => {
        ...
        function renderHltvStatsTable(state) {
            ...
            [['all', '?'], ['CT', 'CT'], ['T', 'T']].forEach(([key, label]) => {
                html += `<button type="button" class="${mode === key ? 'active' : ''}" data-postmatch-stats-mode="${key}">${label}</button>`;
            });
            html += '</div>';
            if (!hasAnyPostmatchStats(state)) return html + renderPostmatchNotice('????????????????????????????? MatchZy CSV?');
            if ((mode === 'CT' || mode === 'T') && !hasSideStatsData(state, mode)) return html + renderPostmatchNotice(`?? ${mode} ???????????????????????`);
            const rows = postmatchStatRows(state, mode);
            const grouped = rows.reduce((acc, row) => {
                const team = row.p.rosterTeam || '???';
                if (!acc[team]) acc[team] = [];
                acc[team].push(row);
                return acc;
            }, {});
            const teamOrder = ['A', 'B', '???'].filter(team => grouped[team]);
            ...
            teamOrder.forEach(team => {
                const label = team === 'A' ? 'A ?' : team === 'B' ? 'B ?' : team;
                html += renderHltvTeamTable(label, grouped[team]);
            });
            html += '</div><p class="postmatch-note">* KAST / Swing / Rating ?????????? HLTV ??????????????????????MatchZy CSV ????</p>';
            return html;
        }
        ...
        function renderKillMatrixTable(state) {
            ...
            let html = '<div class="postmatch-tabs">';
            [['all', '?'], ['opening', 'Opening'], ['awp', 'AWP']].forEach(([key, label]) => {
                html += `<button type="button" class="${mode === key ? 'active' : ''}" data-postmatch-matrix-mode="${key}">${label}</button>`;
            });
            html += '</div>';
            if (!players.length) return html + renderPostmatchNotice('????? SteamID ?????????????');
            const teamA = players.filter(p => p.rosterTeam === 'A');
            const teamB = players.filter(p => p.rosterTeam === 'B');
            if (!teamA.length || !teamB.length) return html + renderPostmatchNotice('?? A/B ?????? SteamID ?????????????');
            ...
            html += '<div class="matrix-wrap"><table class="matrix-table duel-matrix"><tr><th>A ? \\ B ?</th>';
            ...
            html += '</table></div>';
            if (!hasVisibleValue) html += '<p class="postmatch-note">???????????????? 0 ???</p>';
            else html += '<p class="postmatch-note">?????????? A ?????? B ?????????? B ??????? A ?????</p>';
            return html;
        }
        function renderUndercoverTaskReview(state) {
            ...
            let html = '<div class="postmatch-panel"><h3 style="margin-top:0;">??????</h3>';
            undercovers.forEach(p => {
                html += `<div style="margin-top:14px;"><b>${htmlEscape(p.name)} [${p.rosterTeam || '-'}?]</b>`;
        ...
        function renderPostmatchPanels(state) {
            ...
            if (!host) return;
            let html = '<div class="postmatch-panel"><h3 style="margin-top:0;">????</h3>' + renderHltvStatsTable(state) + '</div>';
            html += '<div class="postmatch-panel"><h3 style="margin-top:0;">????</h3>' + renderKillMatrixTable(state) + '</div>';
        ...
        function renderLiveGame(state) {
            ...
            let html = '<div class="live-header" style="background:#263238; color:#fff;">' +
                '<span style="font-size:24px;">???A? <span style="color:#64b5f6;">' + (live.scoreA || 0) + '</span> - <span style="color:#ffb74d;">' + (live.scoreB || 0) + '</span> B?</span> ' +
                '<span style="font-size:16px; color:#b0bec5;">CT/T???' + (live.scoreCT || 0) + ':' + (live.scoreT || 0) + '</span> ' +
                '<span style="font-size:20px; color:#aed581;">? ' + (live.currentRound || 0) + ' ????? ' + (live.winTarget || 13) + ' ?</span> ' +
                '<button onclick="showRules()" style="background:#455a64; color:#fff; border:1px solid #78909c;">?? ??????</button></div>';
            if (live.matchFinished) {
                html += '<div style="background:#fff8e1; border:1px solid #fbc02d; padding:12px; border-radius:6px; margin-bottom:12px; font-weight:bold; color:#e65100;">??????????' + (live.winnerTeam || '?') + '???????????/?????</div>';
            }

            if (undercoverEnabled && currentPlayer?.gameRole === 'Undercover') {
                html += '<h3 style="color:#d32f2f;">??????</h3>';
                ...
                html += '<p><b>???????</b> ' +
                    '<button id="btn-complete" onclick="markComplete()" style="background:#4caf50; color:#fff;">? ????</button> ' +
                    '<button id="btn-undo" onclick="undoComplete()">? ????</button> ' +
                    '<button id="btn-abandon" onclick="abandonTask()" style="background:#9e9e9e; color:#fff;">????</button> ' +
                    '<button id="btn-hint" onclick="requestHint()" style="background:#2196f3; color:#fff;">?? ????</button> ' +
                    '<button id="btn-replace" onclick="replaceTask()" style="background:#9c27b0; color:#fff;">????</button></p>';
                html += '<p style="margin-top:15px;"><b>????(N)???</b> ' +
                    ...
                    '<button id="btn-n-set" onclick="changeN(\'N_SET\')" style="background:#607d8b; color:#fff;">???? N ?</button></p>';
                html += '<hr style="border-top:1px dashed #ffb74d;"><p style="font-size:16px; margin-bottom:0;">???????<span id="selected-cell" style="color:#d32f2f; font-weight:bold; font-size:20px;">?????</span></p>';
                html += '</div>';
                ...
                html += '<h3 style="color:#1565c0; margin-top:0;">????????</h3>';
                html += '<p style="color:#455a64; line-height:1.7;">?????????????????????????????????????????????????????????</p>';
                html += '<textarea id="detective-question" placeholder="???????????????????/????/????????&#10;????????????????????????????" style="width:100%; height:150px; font-size:15px; padding:10px; box-sizing:border-box;"></textarea></div>';
            } else if (currentPlayer?.gameRole === 'Soldier') {
                html += '<div style="text-align:center; padding:40px; background:#e8f5e9; border:1px solid #a5d6a5; border-radius:8px;"><h2 style="color:#2e7d32;">????</h2><p style="font-size:16px; color:#555;">?????????????????????????????</p></div>';
            }

            if (currentPlayer?.role === 'Admin') {
                html += '<hr><div style="background:#eceff1; padding:15px; border-radius:6px; border:1px solid #cfd8dc;">';
                html += '<h4 style="margin-top:0;">?????????????</h4>';
                html += 'A????<input id="admin-scoreA" type="number" value="' + (live.scoreA || 0) + '" style="width:60px;"> &nbsp;&nbsp;';
                html += 'B????<input id="admin-scoreB" type="number" value="' + (live.scoreB || 0) + '" style="width:60px;"> &nbsp;&nbsp;';
                html += 'CT???<input id="admin-scoreCT" type="number" value="' + (live.scoreCT || 0) + '" style="width:60px;"> &nbsp;&nbsp;';
                html += 'T???<input id="admin-scoreT" type="number" value="' + (live.scoreT || 0) + '" style="width:60px;"> &nbsp;&nbsp;';
                html += '?????<input id="admin-round" type="number" value="' + (live.currentRound || 0) + '" style="width:60px;"> &nbsp;';
                html += '<button onclick="updateLiveData()" style="background:#607d8b; color:#fff;">????</button> ';
                html += '<button onclick="resetFormalMatchCounters()" style="background:#d32f2f; color:#fff;">??????????????</button>';
                html += '<p style="font-size:13px;color:#64748b;margin:8px 0 0;">???????????/??/??????????????????????????????????????????????</p></div>';

                if (undercoverEnabled) {
                    html += '<div style="margin-top:15px; padding:15px; background:#e8f5e9; border:1px solid #a5d6a5; border-radius:6px;">';
                html += '<h4 style="margin-top:0; color:#2e7d32;">????????</h4>';
                html += '<p style="color:#455a64; line-height:1.7; margin-top:0;">????????????????????????????????????????????? 2????????? -12 ??</p>';
                ...
                if (detectives.length === 0) {
                    ...
                    html += '<table class="scoreboard-table" style="margin-top:10px;"><tr><th>??</th><th>??</th><th>??????</th><th>????</th><th>??</th></tr>';
                    detectives.forEach(p => {
                        ...
                        html += `<tr>
                            ...
                            <td><button onclick="setDetectiveQuestionCount('${p.playerId}')" style="background:#2e7d32;color:#fff;">??</button></td>
                        </tr>`;
                    });
                    html += '</table>';
                }
                ...
                html += '<div style="margin-top:15px; padding: 15px; background: #fff3e0; border: 1px solid #ffb74d; border-radius:6px;"><h4 style="margin-top:0; color:#e65100;">??????</h4>';
                const undercovers = Object.values(window._allPlayers || {}).filter(p => p.gameRole === 'Undercover');
                if (undercovers.length === 0) {
                    html += '<p style="color:#777;">????????</p>';
                } else {
                    undercovers.forEach(p => {
                        html += `<div style="margin:12px 0 18px;"><b>???${htmlEscape(p.name)} [${p.rosterTeam || '-'}? / ???${getLiveSide(p) || '???'} / ???${getExpectedSide(p, state).label}]</b>`;
                        html += renderTaskGrid(p.taskGrid || {}, { compact: true });
        ...
        function terminateGame() {
            const text = prompt('??????????????????????????????????????? TERMINATE ???');
            if (text !== 'TERMINATE') return alert('???????');
        ...
        function kickPlayer(playerId, name) { if (confirm('????????' + name + '?')) ws.emit('ADMIN_ACTION', { playerId: myPlayerId, action: 'KICK_PLAYER', payload: { playerId } }); }
        ...
        function releaseRoles() { if (confirm('?????????????????????????????')) ws.emit('ADMIN_ACTION', { playerId: myPlayerId, action: 'RELEASE_ROLES' }); }
        ...
        function resetFormalMatchCounters() { if (confirm('?????????????????????? 1 ?????????????/?????????????????')) ws.emit('ADMIN_ACTION', { playerId: myPlayerId, action: 'RESET_FORMAL_MATCH_COUNTERS' }); }
        ...
        function uploadCsv() {
            ...
            if (!file) return alert('????????? MatchZy CSV ???');
        ...
        function doQuit() { const name = document.getElementById('quit-name-input').value.trim(); if (!name) return alert('???????'); ws.emit('PLAYER_QUIT', { playerId: myPlayerId, confirmName: name }); document.getElementById('quit-modal').style.display = 'none'; }
        ...
        window.handleCellClick = function (cellId) {
            ...
            document.getElementById('selected-cell').textContent = `??? ${cellId}?`;
            window.selectedCellId = cellId;
            updateButtonStates();
        };
        window.markComplete = function () { const cellId = window.selectedCellId; if (!cellId) return alert('?????????'); ws.emit('TASK_ACTION', { playerId: myPlayerId, action: 'MARK_COMPLETE', cellId }); };
        window.undoComplete = function () { const cellId = window.selectedCellId; if (!cellId) return alert('?????????'); ws.emit('TASK_ACTION', { playerId: myPlayerId, action: 'UNDO_COMPLETE', cellId }); };
        window.abandonTask = function () { const cellId = window.selectedCellId; if (!cellId) return alert('?????????'); ws.emit('TASK_ACTION', { playerId: myPlayerId, action: 'ABANDON', cellId }); };
        window.requestHint = function () { const cellId = window.selectedCellId; if (!cellId) return alert('?????????'); ws.emit('TASK_ACTION', { playerId: myPlayerId, action: 'REQUEST_HINT', cellId }); };
        window.replaceTask = function () { const cellId = window.selectedCellId; if (!cellId) return alert('?????????'); ws.emit('TASK_ACTION', { playerId: myPlayerId, action: 'REPLACE', cellId }); };
        window.changeN = function (action) {
            const cellId = window.selectedCellId;
            if (!cellId) return alert('???????N???');
            let nValue = undefined;
            if (action === 'N_SET') {
                const n = prompt('?????? N ? (??):');
                if (n === null) return;
                nValue = parseInt(n);
                if (isNaN(nValue)) return alert('???????');
            }
        ...
        function openTemplateModal() {
            if (!window._currentTaskTemplate) return alert("??????????????...");
        ...
        function renderTplPreview() {
            ...
            ['A1', 'A2', 'A3', 'B1', 'B2', 'B3', 'C1', 'C2', 'C3'].forEach(id => {
                ...
                const nInfo = (cell.nType && cell.nType !== 'none') ? `<div style="color:#d32f2f; font-size:14px; margin-top:8px; font-weight:bold;">[?N????]</div>` : '';
            ...
            document.getElementById('tpl-prev-rep-desc').innerHTML = `<span class="task-desc">${rep.description || ''}</span> <span style="color:#888;">(?? ${rep.level || 4})</span>`;
        }
        ...
        window.selectTplCell = function (id) {
            ...
            if (id === 'rep') {
                const rep = window._editingTemplate.replacementTask;
                document.getElementById('tpl-edit-title').innerHTML = `?? ?????<b style="color:#7030a0;">????</b>`;
                ...
                const cell = window._editingTemplate.cells[id];
                document.getElementById('tpl-edit-title').innerHTML = `?? ?????<b style="color:#1976d2;">?? ${id}</b>`;
        ...
        function loadTemplateFromLocal() {
            ...
            try {
                ...
                const savedAt = localStorage.getItem('caorenCupTaskTemplateBackupAt') || '????';
                alert('???????????????' + savedAt);
            } catch (e) {
                alert('?????????' + e.message);
            }
        ...
        function saveTemplateFromJson() {
            try {
                ...
                if (!parsed.cells || !parsed.lines || !parsed.replacementTask) throw new Error("JSON ???????? cells / lines / replacementTask");
                if (!confirm("JSON ?????????????????????")) return;
        ...
        function showRules() {
            ...
            if (role === 'Undercover') {
                rules = [
                    '<b style="color:#d32f2f;font-size:20px;">????? 1/2?</b><br/><br/>' +
                    '1. ?????????????????????????????????<br/>' +
                    '2. ?????????????????????????????????????????<br/>' +
                    '3. ???????????????????<br/>' +
                    ' - <b style="color:green">????</b>???? / N????<br/>' +
                    ' - <b style="color:orange">????</b>?N??????(????)<br/>' +
                    ' - <b style="color:blue">????</b>??????<br/>' +
                    ' - <b style="color:purple">????</b>????????<br/>' +
                    ' - ???????????',
                    '<b style="color:#d32f2f;font-size:20px;">????? 2/2?</b><br/><br/>' +
                    '4. N ??????<br/>' +
                    ' - [?????]?????????????<br/>' +
                    ' - [?????]????????????<br/>' +
                    '5. ???????<br/>' +
                ...
                rules = [
                    '<b style="color:#1976d2;font-size:20px;">??????</b><br/><br/>' +
                    '1. ?????????????????????<br/>' +
                    '2. ???????? 45 ?????????? 2 ????<br/>' +
                    '3. ????????????????????????<br/>' +
                    '4. ???????????????????????????????'
                ...
                rules = [
                    '<b style="color:#388e3c;font-size:20px;">??????</b><br/><br/>' +
                    '1. ???????????????????<br/>' +
                    '2. ????????????<br/>' +
                    (window._undercoverModeEnabled === false ? '3. ???????????????????????' : '3. ????????????????????')
                ];
            }
            ...
            const renderPage = function () {
                ...
                nav.innerHTML = '';
                if (rulesCurrentPage > 0) nav.innerHTML += '<button onclick="changeRulesPage(-1)" style="font-size:16px;">?? ?????</button> ';
                if (rulesCurrentPage < rules.length - 1) nav.innerHTML += '<button onclick="changeRulesPage(1)" style="font-size:16px; background:#e0e0e0;">??? ??</button>';
